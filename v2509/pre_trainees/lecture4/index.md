...
>
> [C语言文件简易操作教程](./C语言文件操作——二进制操作教程.md)
>
# 最后的终端改进
...
> # **功能追加与**参数处理
>
> 学习了这么多的命令行工具（它们也是程序），你早就发现了大部分命令行工具都有参数处理的能力。比如我们使用的`ls -a`，其中`-a`就是它的参数，`ls`的输出就会因为有`-a`的存在而不同。为了进一步完善我们的模拟终端，这次我们也要为自己的终端新增参数处理的能力，让我们自定义的不同命令的输出能够随参数的不同而变化。具体要求查看后方文档：[最后的终端改进](./最后的终端改进.md)
>
# Git
...
> # C语言学习——文件操作之二进制
>
> 阅读[C语言文件操作——二进制操作教程](./C语言文件操作——二进制操作教程.md)并完成内部和下方的必做练习。
