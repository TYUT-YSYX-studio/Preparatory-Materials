...
不仅如此，我们还加了几条注释，将最终程序的行为和某些汇编代码联系了起来（比如赋值操作实际就是内存写入），这也有助于你理解[上一节讲义开头提到的内存分配顺序](/pre_trainees/lecture3/指针_结构体教程.md)。当然前提还是你要能读懂汇编代码。如果现在就你对此感兴趣可以尝试使用这个网站：[Compiler Explorer](https://godbolt.org/)
...
每次都要自己重新做一遍汇编操作还是太麻烦了，你还记得我们前面学过的[<strong>source命令</strong>](/pre_trainees/lecture3/index.md)吗？尝试用source命令生成目标文件和最终的可执行文件吧。其中源文件有三个，文件名在左上角，且都在同一目录下：
...
>
> 1. 这个文件需要是可读的，因为我们要用[<strong>source命令</strong>](/pre_trainees/lecture3/index.md)或者` . `指令来执行这个“脚本”。
> 
...
> 
>
> 注意灵活使用你在前面学到的结构体有关知识，比如[结构体的大小](/pre_trainees/lecture3/指针_结构体教程.md)。还有文件操作有关的知识，比如文件打开模式。
>
