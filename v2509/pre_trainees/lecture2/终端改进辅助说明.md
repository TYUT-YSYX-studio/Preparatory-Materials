...
<strong>（使用WSL的同学可以直接跳过，你应该STFW）</strong>

- 正常情况下你可以直接复制文件到虚拟机，但不一定可以从虚拟机复制到主机
    - Linux的复制是`Ctrl + C`，粘贴是`Ctrl + V`（终端里是`Ctrl + Shift + C` `Ctrl + Shift + V`）
    - 具体的就是在Windows里`Ctrl + C`后在Linux文件夹界面`Crtl + V`
- 如果无法复制，在终端输入以下命令：`$ sudo apt install open-vm-tools-desktop -y    # sudo需要你再输入你刚刚设置的密码`

检查客户机隔离选项（编辑虚拟机设置->选项）

![](../static/GuGKbiejiodDddxt5hAcaLeSnof.png)
...
根据以下链接学习字符编码与乱码原因相关内容，<strong>之后提交的源码都要使用UTF-8编码</strong>（<strong>Linux默认就是</strong>，Devcpp默认是GB2312/GBK编码）。如果使用不匹配的编码打开文件会乱码（比如说使用UTF-8编码方式打开GBK编码的文件）。
...
![](../static/VeztbTi9dorrX7xYMndcKC2TnIF.png)

从GBK转成UTF-8：

用 通过GBK模式重新打开，再用 通过UTF-8保存，关掉你的代码文件，再重新打开就好。

![](../static/LtYKborzbofBmOxpxckcKDspnPd.png)
