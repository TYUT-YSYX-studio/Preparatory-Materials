...
![QQ_1729943815653](static/QQ_1729943815653.png)

2. 创建一个自己的副本，其中表格内容自己可以更改，随后切换权限到所有人可查看

![QQ\_1729944755744\.png](static/QQ_1729944755744.png)
...
[C语言文件简易操作教程](subdocument/C语言文件操作——二进制操作教程.md)

# 最后的终端改进

# **功能追加与****参数处理**

学习了这么多的命令行工具（它们也是程序），你早就发现了大部分命令行工具都有参数处理的能力。比如我们使用的`ls -a`，其中`-a`就是它的参数，`ls`的输出就会因为有`-a`的存在而不同。为了进一步完善我们的模拟终端，这次我们也要为自己的终端新增参数处理的能力，让我们自定义的不同命令的输出能够随参数的不同而变化。具体要求查看后方文档：[最后的终端改进](subdocument/最后的终端改进.md)
...
4. 最后将你的整个`try_git`文件夹**复制**到作业提交文件夹中，等待作业提交。
...
其中`argc`和`argv`的使用你需要查看[C语言官方文档（节选pdf）](https://xcnlirxdrdxr.feishu.cn/wiki/NAT3wppJEi4VhTkeG2rceeAXn4g)（或者后方的中文翻译版）获取，只需要阅读`5.1.2.3.2 Program startup`中与`argc`和`argv`有关的部分内容，**并结合****STFW**获取答案。同时不要忘记你所学过的指针数组相关知识，这样你才能更好地阅读这个文档。
...
然后将**你的程序源代码重命名为**`C_echo`并放入`姓名-专业班级-Great`**`-4`**文件夹等待作业提交。
...
>
> 3. 将文件夹**压缩**并提交到[第四次作业提交表单](https://fa45epzd9c7.feishu.cn/share/base/form/shrcnkET92LbdskA1j9ttZCRtLh)。
>
> 4. 如果你学有余力完成了下面的拔高内容，则把文件夹重命名，格式为`姓名-专业班级`**`-NewStar-4`**。不要忘记更新你的源代码和截图，还有发送邮件。
>
> 5. 支线任务不要求提交，但你仍可按格式提交作业压缩包。
>
> 本周作业提交截止时间：**11月15号晚23:00**（如果因为自身原因未完成本周作业，在表单**请假说明**一栏填写原因，并写上你的预计提交时间即可）
>
# 拔高
...
以**`GNU/Linux`**上的编译路径为例，C语言代码的编译要经过四个步骤：**预处理、编译、汇编和链接**，其中前两个步骤中产生的文件都是我们人类能够看懂的，尽管会比源代码更加难以理解。
...
检查后还有一步操作才能生成`.s`文件，那就是翻译。检查后成功进入翻译环节的话，编译器就会把我们的代码文件根据不同**指令集平台**翻译成汇编代码并生成`.s`临时文件。

我们可以利用`-S`选项生成并保留`.s`文件。**从源代码（\.c文件）开始就可以**。这次会生成文件而不是输出到终端：
...
main:
        ...
        li        a5,9          # 将a5寄存器赋值为9
        sw        a5,-20(s0)    # 将**变量a**赋值为9
        li        a5,57         # 将a5寄存器赋值为57，也就是'9'
        sb        a5,-21(s0)    # 将**变量ch**赋值为'9'
...
不仅如此，我们还加了几条注释，将最终程序的行为和某些汇编代码联系了起来（比如赋值操作实际就是内存写入），这也有助于你理解[上一节讲义开头提到的内存分配顺序](https://xcnlirxdrdxr.feishu.cn/wiki/FUVow6xjWiwFOXkTY2KcYI7Knnd#share-Sq3ZdoZEQojSk4xYHTacUz8AnNg)。当然前提还是你要能读懂汇编代码。如果现在就你对此感兴趣可以尝试使用这个网站：[Compiler Explorer](https://godbolt.org/)
...
汇编器能将汇编代码（`example.s`）转换成目标文件（`example.o`），即机器能够直接识别的**二进制代码**。用gcc的\-c选项即可获得：
...
刚刚提到的[合并多个目标文件](https://xcnlirxdrdxr.feishu.cn/wiki/KF1FwCebBidqk1kr2dTcdmy5nne#share-MC0kdy2FQoBIYNxVin7c1mcSnoe)其实就是我们能实现多文件编程的诀窍。尽管可能有同学已经试过用直接用gcc把所有源代码文件放到参数列表，最后成功生成可执行文件。但在以后代码量越来越多，代码文件也越来越多的时候，这样的方法就不好用了。**尤其是当你需要排除某些源代码文件时**。

因此把最终的目标文件（汇编后但还未链接的`.o`文件）集中起来，检查哪些目标文件需要改动，更新需要改动的目标文件再集中链接才是做**大型项目**的一个更好的选择。

可要如何检查目标文件是否要更新呢？其中一个办法是检查目标文件的更新时间和源代码的更新时间是否一致。不过我们不要求你现在就掌握这些。现在就先只做到前半部分吧。

每次都要自己重新做一遍汇编操作还是太麻烦了，你还记得我们前面学过的[**source命令**](https://xcnlirxdrdxr.feishu.cn/wiki/BrNOwto7CimkFekA3svcpPzMnYc#share-B7t2dUhdaoip4QxCsKUcWxmInGf)吗？尝试用source命令生成目标文件和最终的可执行文件吧。其中源文件有三个，文件名在左上角，且都在同一目录下：
...
1. 这个文件需要是可读的，因为我们要用[**source命令**](https://xcnlirxdrdxr.feishu.cn/wiki/BrNOwto7CimkFekA3svcpPzMnYc#share-B7t2dUhdaoip4QxCsKUcWxmInGf)或者` . `指令来执行这个“脚本”。
...
阅读[C语言文件操作——二进制操作教程](subdocument/C语言文件操作——二进制操作教程.md)并完成内部和下方的必做练习。
...
1. 像终端一样，你要实现像这样的指令：`r <name> <age>`
    ...
    3. 将获得的**结构体**的数据用fwrite写入当前目录下的build/list\.dat文件路径，如果没有build路径记得自己创建
...
注意灵活使用你在前面学到的结构体有关知识，比如[结构体的大小](https://xcnlirxdrdxr.feishu.cn/wiki/FUVow6xjWiwFOXkTY2KcYI7Knnd#share-KV5xdfIyBoGntOx2FmfcAqrxnge)。还有文件操作有关的知识，比如文件打开模式。
