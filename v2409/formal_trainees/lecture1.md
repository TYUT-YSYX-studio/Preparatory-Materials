...
main:
    pushl   %ebp          # 保存调用者的栈基指针（设置新栈帧）
    movl    %esp, %ebp    # 将当前栈指针存储到栈基指针（设置栈帧基准）
    movl    $0, %eax      # 将返回值 0 加载到 eax 寄存器（函数返回值寄存器）
    popl    %ebp          # 恢复调用者的栈基指针
    ret                   # 返回到调用者
    .size   main, .-main  # 计算 main 函数的大小
```
...
| 选项 | 功能 |
|------|------|
| `-o &lt;file&gt;` | 指定输出文件名称，默认为 `a.out`（不指定的情况下） |
| `-E` | 仅进行预处理，输出 `.i` 文件 |
| `-S` | 仅生成汇编代码，输出 `.s` 文件 |
| `-c` | 仅生成目标文件（不链接），输出 `.o` 文件 |
| `-Wall` | 启用所有警告（`warning`） |
| `-Werror` | 将所有警告（`warning`）转化为错误（`error`） |
| `-g` | 启用调试信息，配合 `gdb` 使用 |
| `-fsanitize=address` | 启用 ASAN，一个运行时内存错误检查工具 |
| `-fsanitize=undefined` | 启用 UBSAN，一个运行时未定义行为（**U**ndefined **B**ehavior）检查工具 |
| `-O0/-O1/-O2/-O3` 等 | 启用不同级别的优化，`-O3` 为最高优化等级，`-O0` 不进行任何优化 |
...
> [!TIP]
> 也可以自己写多个多文件执行程序练手，比如改进你的终端.c：main.c （主函数 main 其中调用其他函数），lib.c（存放小型函数比如 list，help，do 等功能） ，add_function（存放另外函数比如排序 sort 等其他程序） 三个 C 语言文件，最后通过 make run 运行

2.写一篇关于 **makefile** 学习的markdown **笔记**，其中**涵盖一些图片**，将该 md 文件交回，我们会检查相关图床配置是否正确

~~将以上两部分打包到大文件夹并压缩，命名为“专业班级-姓名”交回到邮箱~~
